...
import com.uwrite.exception.UwriteApiException;
import com.uwrite.model.Rsp;
...
/**
 * Class provides functionality of https://www.uwrite.proctoru.com/api/v2
 */
public final class Uwrite {

	private static final String API_URI = "https://uwrite.proctoru.com/api/v2";
	...
	/**
	 * Constructs Uwrite Api Client. Requires api key and secret
	 *
	 * @param apiKey    get key on https://uwrite.proctoru.com
	 * @param apiSecret get secret on https://uwrite.proctoru.com
	 */
	public Uwrite(String apiKey, String apiSecret) {
		...
		Client client = ClientBuilder
				...
				.build();
		this.target = client.target(API_URI);
	}
	 ...
	 * This method has difference - <b>Uwrite</b> will send check info to callback url
	 *
	 * @param id                file id
	 * @param type              check type
	 ...
	 *
	 * @param response
	 * @throws UwriteApiException if response contains any errors
	 */
	private void checkResponse(Rsp response) throws UwriteApiException {
		Objects.requireNonNull(response);
		if (!response.isSuccessful()) {
			throw new UwriteApiException(Arrays.toString(response.getErrors()));
		}
	}
